<script lang="ts">
	import type { SchemaCategory, SchemaControl, SchemaControlInScope, SchemaEvaluationResult, SchemaUser } from '$lib/api/openapi/orchestrator';
	...
	{#if open}
		<div class="divide-y divide-gray-100 border-t border-gray-100 px-4">
			{#each controls as control}
				<ControlRow
					{control}
					evaluation={evaluationByControl[control.id ?? '']}
					{evaluationByControl}
					{allEvaluations}
					{assessmentCountByMetric}
					{assessmentById}
					{controlInScopeByControlId}
					controlInScope={controlInScopeByControlId[control.id ?? '']}
					{auditScopeId}
					{targetId}
					{users}
				/>
