<script lang="ts">
	...
	async function create() {
		...
		const { error: apiError } = await client.POST('/v1/orchestrator/audit_scopes', {
			body: {
				name,
				targetOfEvaluationId: data.toe.id!,
				catalogId: selectedCatalog.id,
				status: 'AUDIT_SCOPE_STATUS_SETUP'
			}
		});
		if (apiError) {
			errorMsg = (apiError as { message?: string }).message ?? 'Failed to create audit scope';
			saving = false;
			return;
		}
