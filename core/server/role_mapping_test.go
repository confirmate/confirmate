...
func TestKeycloakRoleMapper(t *testing.T) {
	tests := []struct {
		name string
		in   string
		want string
		...
		{name: "unknown role is passed through", in: "AMOE_ADMIN", want: "AMOE_ADMIN"},
		{name: "whitespace is trimmed", in: "  admin  ", want: orchestrator.RoleAdmin},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, KeycloakRoleMapper(tt.in))
		})
	}
}
