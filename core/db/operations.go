...
// Create attempts to insert the provided record into the database. If a constraint violation occurs, it returns
// ErrUniqueConstraintFailed or ErrConstraintFailed, depending on the error message.
...
// Save attempts to save the given record to the database, applying optional conditions for filtering.
// If a constraint violation occurs, it returns ErrConstraintFailed.
...
// Update applies the provided changes to the database record, optionally applying conditions for filtering.
// Returns ErrConstraintFailed on a constraint violation or ErrRecordNotFound if no matching record is found.
...
// Delete attempts to delete the record with the given ID from the database. Returns ErrRecordNotFound if no matching
// record is found.
func (s *Storage) Delete(r any, conds ...any) error {
	// Remove record r with a given I D
...
// Raw executes a raw SQL query and scans the result into the provided destination. Returns an error if the query fails.
